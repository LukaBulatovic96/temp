...
const reorderPerlin = () => {
  girdBAW.value = generatePerlin(
    size.value,
    noise.value,
    startingDimension.value,
    interpolation.value
  );
...
const drawBlackAndWhite = () => {
  ...
  for (let i = 0; i < girdBAW.value.length; i++) {
    for (let j = 0; j < girdBAW.value[i].length * 4; j += 4) {
      imgDataBAW.value.data[i * size.value * 4 + j + 0] =
        girdBAW.value[i][j / 4];
      imgDataBAW.value.data[i * size.value * 4 + j + 1] =
        girdBAW.value[i][j / 4];
      imgDataBAW.value.data[i * size.value * 4 + j + 2] =
        girdBAW.value[i][j / 4];
...
const draw = () => {
  ...
  for (let i = 0; i < grid.value.length; i++) {
    for (let j = 0; j < grid.value[i].length * 4; j += 4) {
      imgData.value.data[i * size.value * 4 + j + 0] = parseInt(
        grid.value[i][j / 4].substring(0, 2),
        16
      );
      imgData.value.data[i * size.value * 4 + j + 1] = parseInt(
        grid.value[i][j / 4].substring(2, 4),
        16
      );
      imgData.value.data[i * size.value * 4 + j + 2] = parseInt(
        grid.value[i][j / 4].substring(4, 6),
        16
      );
  ...
  <div class="container w-50">
    ...
    <div class="row my-1">
      ...
      <div class="col-sm ps-5">
        <div
          :class="interpolation ? 'dot-clicked' : 'dot'"
          @click="interpolation = !interpolation"
        ></div>
      </div>
      <div class="row my-1">
