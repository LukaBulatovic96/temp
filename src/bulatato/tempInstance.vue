...
const resetEnemy = () => {
  const delta_X = Math.random() * (gameSettings.value.width-enemy.value.radius*2)+enemy.value.radius;
  const delta_Y = Math.random() *(gameSettings.value.height-enemy.value.radius*2)+enemy.value.radius;
  enemy.value.position.x = delta_X;
  enemy.value.position.y = delta_Y;
};
